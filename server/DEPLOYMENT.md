...
Instagram publishing uses the Meta Instagram Graph API Reels content publishing flow, but video preparation now happens during the backend upload phase. The backend creates a strict `output_instagram.mp4` with `+faststart`, upserts it in the project Google Drive root, grants public read access, and sends its direct download URL to the VPS as `instagram.prepared_video_url`.

At slot time, the VPS creates the Instagram container. For binary `rupload.facebook.com` uploads, it downloads `prepared_video_url` into `data/tmp/instagram`, validates the MP4, uploads the bytes, and deletes the local temp file in all success/failure paths. If Meta reports a zero-byte/phase-error rupload ingest (`uploading_phase=error`, `bytes_transferred=0`), the publisher falls back to a fresh `video_url` container using the same Drive-prepared URL. The Drive `output_instagram.mp4` is intentionally retained for retry/audit until the project is re-exported or deleted. The VPS persists the current container id, upload URI, upload method, last status, and publish stage in `jobs.json`; if a polling attempt times out, the next scheduler tick resumes that same unexpired healthy container without downloading the file again.
