...
async def test_dispatch_tiktok_quota_error_fails_terminally_at_normal_cap(
    ...
    """Quota errors get NO extended spaced-retry window (owner decision
    2026-07-24): they burn the normal attempt budget and fail fast so the
    terminal Discord ping arrives in time for a manual upload near the slot."""
    settings = replace(
        _settings_for(example_yaml, tmp_server_dir / "avatars"), pfm_api_key="key"
    )
    store = JobStore(tmp_path / "jobs.json")
