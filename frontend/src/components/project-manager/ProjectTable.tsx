...
interface ProjectTableProps {
  ...
  onPreview: (target: { driveVideoId: string | null; projectId: string }) => void;
...
const COLUMNS: { key: SortColumn | null; label: string; className: string }[] = [
  ...
  { key: null,              label: "Config",       className: "w-40" },
    ...
    <>
      {Array.from({ length: 5 }).map((_, i) => (
        <tr key={i} className="border-b border-[hsl(var(--border))]/50">
          ...
          <td className="py-3 pr-3"><div className="h-3 w-28 rounded bg-[hsl(var(--muted))] animate-pulse" /></td>
          ...
          {colCount > COLUMNS.length && <td className="py-3 pr-3"><div className="h-4 w-4 rounded bg-[hsl(var(--muted))] animate-pulse" /></td>}
        </tr>
      ))}
    </>
  );
